...
@synthesize chairPasswordField = _chairPasswordField;
- (id)init {
    if (self = [super init]) {
        _isImmediately =YES;
...
- (void)makeItemCellsIndexs:(NSMutableDictionary *)itemsDic
{
    NSUInteger index = 0;
    // SECTION 1
    NSMutableArray *section0 = [NSMutableArray arrayWithObjects:@"ConfSubject", @"ConfTime", @"ConfMediaType", @"Password", nil];
...
- (void)makeItemCellModelsDic:(NSMutableDictionary *)itemsDic
{
    ...
    TableViewCellModel *cellModel6 = [[TableViewCellModel alloc] initWithKey:@"Password"
                                                                          ConfigureBlock:@selector(configurePasswordCell:forKey:)
                                                                                  action:nil];
    [itemsDic setObject:cellModel6 forKey:cellModel6.strKey];
...
-(void)configurePasswordCell:(UITableViewCell *)cell forKey:(NSString *)str
{
    cell.textLabel.text = str;
    cell.textLabel.font = CELL_TITLE_FONT;
    cell.textLabel.textColor = [UIColor blackColor];
    ...
    ESpaceConfTableViewCell *confCell = (ESpaceConfTableViewCell *)cell;
    _chairPasswordField = confCell.inputTextField;
    
    _chairPasswordField.delegate = self;
    
    _chairPasswordField.backgroundColor = [UIColor clearColor];
    _chairPasswordField.font = CELL_CONTENT_FONT;
    _chairPasswordField.textColor = [UIColor blackColor];;
    _chairPasswordField.placeholder = NSLocalizedString(@"tap_input_password", @"点击输入主席密码");
    _chairPasswordField.textAlignment = NSTextAlignmentRight;
    _chairPasswordField.clearButtonMode = UITextFieldViewModeWhileEditing;
    [self updateTableViewCellStyle:cell enabled:NO];
    cell.userInteractionEnabled = YES;
}
...
- (void)hideKeyboard
{
    ...
    if ([self.chairPasswordField isFirstResponder]) {
        [self.chairPasswordField resignFirstResponder];
    }
}
...
- (void)createConfEnter{
    BOOL isCreateConfSuccessed = [[ManagerService confService] createConferenceWithAttendee:_selectedAttendeArray mediaType:_confMediaType subject:self.confSubjectTextField.text startTime:_isImmediately ? nil : self.beginDate confLen:_isImmediately ? 0 : self.lastInterval/60 password:self.chairPasswordField.text];
