...
//        NSArray *array = [[ManagerService callService].sipAccount componentsSeparatedByString:@"@"];
        self.selfNumber = confInfo.selfNumber;
...
- (VCConfUpdateInfo *)selfConfInfo {
    for (VCConfUpdateInfo *tempAttendee in self.confStatus.participants)
    {
        if ([tempAttendee.aucName isEqualToString:self.selfNumber])
        {
            return tempAttendee;
        }
    }
...
- (BOOL)isSelfMaster {
    ...
    VCConfUpdateInfo *selfAttendee = nil;
    ...
    return selfAttendee.isChair;
}

- (BOOL)isUsmDataConf {
    return (self.confStatus.media_type == CONF_MEDIATYPE_DATA
            && ![[ManagerService confService] isUportalSMCConf]
